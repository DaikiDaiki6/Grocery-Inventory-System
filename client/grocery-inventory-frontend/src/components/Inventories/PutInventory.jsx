...
function PutInventory() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await putInventory.mutateAsync({
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {putInventory.isPending ? "Updating..." : "Update Inventory"}
        </button>
      </form>
      ...
      {putInventory.isError && (
        <div
          className={`p-4 border rounded-lg mt-6 ${
            getErrorStyling(putInventory.error).container
          }`}
        >
          <p className="flex items-center gap-2">
            {getErrorStyling(putInventory.error).icon} Error updating inventory:{" "}
            {getErrorMessage(putInventory.error, "updating", "inventory")}
          </p>
        </div>
      )}
    </div>
