...
function GetAllSuppliers() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetAllSuppliers(pageNumber, pageSize);
  ...
  if (error) {
    ...
    return (
      ...
      >
        <p>
          {errorStyling.icon} Error loading suppliers:{" "}
          {getErrorMessage(error, "loading", "supplier")}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 bg-white rounded-2xl shadow-md space-y-4">
      ...
      <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">
        <p>
          Showing {suppliers.length} of {pagination.totalCount} suppliers (Page{" "}
